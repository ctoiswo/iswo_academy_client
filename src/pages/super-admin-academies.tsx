...
export default function SuperAdminAcademies() {
  ...
  const [gracePeriodAcademy, setGracePeriodAcademy] =
    useState<AcademyOverview | null>(null)
  ...
  const subscriptionType = (academy: AcademyOverview) => {
    const {
      ...
      subscription_ever_paid,
    } = academy
    if (!admin_subscription_active)
      return subscription_ever_paid ? 'expired' : 'unpaid'
  ...
  const handleActivateSubscription = async (academy: AcademyOverview) => {
    try {
      const result = await superAdminApi.updateSubscription(
        academy.slug,
        'force_active'
      )
  ...
  const handleDeactivateSubscription = async (academy: AcademyOverview) => {
    if (
      !window.confirm(
        `¿Revocar la activación de pago de "${academy.name}"? El estado de la academia no cambiará.`
      )
    )
      return
    try {
  ...
  const handleConfirmGracePeriod = async () => {
    ...
    const days = Math.ceil(
      (selectedDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    )
    if (days <= 0) {
  ...
  const handleCancelSubscription = async (academy: AcademyOverview) => {
    if (
      !window.confirm(
        `¿Cancelar la suscripción de "${academy.name}"? La academia seguirá operativa sin fecha de expiración.`
      )
    )
      return
    try {
        ...
        <DialogContent className='sm:max-w-md'>
          ...
          <div className='space-y-4 py-2'>
            <p className='text-muted-foreground text-sm'>
              La academia tendrá acceso completo hasta la fecha seleccionada. Al
              vencer, la suscripción expirará automáticamente.
            </p>
            <div className='space-y-2'>
              ...
              <DatePicker
                selected={
                  gracePeriodDate
                    ? new Date(gracePeriodDate + 'T00:00:00')
                    : undefined
                }
                onSelect={(date) =>
                  setGracePeriodDate(
                    ...
                    <div className='space-y-1'>
                      <div className='flex items-center gap-2'>
                        ...
                        {academy.admin_subscription_active ? (
                          academy.subscription_days_remaining !== null &&
                          academy.subscription_days_remaining !== undefined &&
                          academy.subscription_days_remaining <= 30 ? (
                            <Badge
                              variant='outline'
                          ...
                          academy.status === 'active' && (
                            <>
                              ...
                              <Button
                                size='sm'
                                variant='outline'
                                className='h-6 border-amber-500 px-2 text-xs text-amber-600 hover:bg-amber-50'
                                onClick={(e) => {
                                  e.stopPropagation()
                                  setGracePeriodAcademy(academy)
                                  setGracePeriodDate('')
                                }}
                              >
                        ...
                        {academy.admin_subscription_active &&
                          !academy.subscription_ever_paid && (
                            <Button
                              size='sm'
                              variant='outline'
                              className='border-destructive text-destructive hover:bg-destructive/10 h-6 px-2 text-xs'
                              onClick={(e) => {
                                e.stopPropagation()
                                handleDeactivateSubscription(academy)
                              }}
                            >
                      ...
                      {academy.subscription_expires_at && (
                        <div className='text-muted-foreground flex items-center gap-1 pl-1 text-xs'>
                          ...
                          <span>
                            Vence:{' '}
                            {formatExpiryDate(academy.subscription_expires_at)}
                          </span>
                        </div>
                      )}
                      {academy.admin_subscription_active &&
                      ...
                      <TableCell>
                        <div className='space-y-1'>
                          <div className='flex items-center gap-1.5'>
                            {academy.admin_subscription_active ? (
                              academy.subscription_days_remaining !== null &&
                              academy.subscription_days_remaining !==
                                undefined &&
                              academy.subscription_days_remaining <= 30 ? (
                                <Badge
                                  variant='outline'
                            ...
                            academy.status === 'active' && (
                              <>
                                ...
                                <Button
                                  size='sm'
                                  variant='outline'
                                  className='h-7 border-amber-500 text-xs text-amber-600 hover:bg-amber-50'
                                  onClick={() => {
                                    setGracePeriodAcademy(academy)
                                    setGracePeriodDate('')
                                  }}
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align='end'>
                            ...
                            {academy.admin_subscription_active && (
                              <DropdownMenuItem
                                onClick={() =>
                                  handleExtendSubscription(academy)
                                }
                              >
                            ...
                            {academy.admin_subscription_active && (
                              <DropdownMenuItem
                                variant='destructive'
                                onClick={() =>
                                  handleCancelSubscription(academy)
                                }
                              >
                                <Ban className='mr-2 h-4 w-4' />

...
export function LearningPathCertificates() {
  ...
  const { data: certificateConfig, isLoading: isLoadingConfig } = useLearningPathCertificateConfiguration(academySlug, learningPathSlug)
  const { data: certificatesData, isLoading: isLoadingCertificates } = useLearningPathCertificates(academySlug, learningPathSlug, currentPage)
  const updateConfiguration = useUpdateLearningPathCertificateConfiguration(academySlug, learningPathSlug)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{certificateConfig.statistics.total_issued}</div>
            <p className="text-xs text-muted-foreground">certificados en total</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{certificateConfig.statistics.active_certificates}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Certificados Revocados</CardTitle>
            <Award className="h-4 w-4 text-red-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{certificateConfig.statistics.revoked_certificates}</div>
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              <Label htmlFor="enable_certificates" className="flex items-center gap-2">
                <Award className="h-4 w-4" />
                Emitir Certificados
              </Label>
              <p className="text-sm text-muted-foreground">
                Los estudiantes recibirán un certificado al completar todos los cursos
              </p>
            </div>
            <Switch
          ...
          {certificateConfig.certificate_template ? (
            <>
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">{certificateConfig.certificate_template.name}</span>
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  <p>• Título: Certificado de Completado - {certificateConfig.learning_path.title}</p>
                  <p>• Academia: {certificateConfig.academy.name}</p>
                  <p>• Duración Total: {certificateConfig.learning_path.estimated_duration_hours} horas</p>
                  <p>• Cursos Incluidos: {certificateConfig.learning_path.courses_count}</p>
                </div>
              </div>
            </>
          ) : (
            <div className="border-2 border-dashed rounded-lg p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Contacta al administrador de la academia para configurar una plantilla
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Lista de certificados emitidos para esta ruta ({certificatesData?.meta.total_count || 0})
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingCertificates ? (
            ...
            <div className="space-y-4">
                ...
                <TableBody>
                  {certificatesData.data.map((certificate) => (
                    <TableRow key={certificate.id}>
                      <TableCell>
                        <div>
                          <p className="font-medium">{certificate.user.full_name}</p>
              ...
              {certificatesData.meta.total_pages > 1 && (
                <div className="flex items-center justify-between">
                  <p className="text-sm text-muted-foreground">
                    Página {certificatesData.meta.current_page} de {certificatesData.meta.total_pages}
                  </p>
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={certificatesData.meta.current_page === certificatesData.meta.total_pages}
                    >
                      Siguiente
                    </Button>
                  </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-xs text-muted-foreground">
                Los certificados se emitirán automáticamente cuando los estudiantes completen la ruta
              </p>
            </div>
          )}
        </CardContent>

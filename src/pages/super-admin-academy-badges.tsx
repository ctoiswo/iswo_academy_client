...
export default function SuperAdminAcademyBadges() {
  ...
  const { slug } = useParams({ from: '/_authenticated/super-admin/gamification/academies/$slug/badges' })
  ...
  const fetchBadges = async (isInitialLoad = false) => {
    try {
      ...
      const data = await superAdminGamificationService.getAcademyBadges(slug, filters)
      setBadges(data)
  ...
  const handleToggleActive = async (badge: BadgeDetail) => {
    ...
    setBadges((prev) =>
      prev.map((b) =>
        b.id === badge.id ? { ...b, is_active: newStatus } : b
      )
    )
    ...
    try {
      ...
      toast.success(
        newStatus ? 'Badge activado correctamente' : 'Badge desactivado correctamente'
      )
      ...
      toast.error(
        error.response?.data?.message || 'No se pudo cambiar el estado del badge'
      )
    }
  }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">En esta academia</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Badges Activos</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Badges Secretos</CardTitle>
            <Eye className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.secret}</div>
            <p className="text-xs text-muted-foreground">Ocultos hasta ganarlos</p>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mb-4 flex flex-col sm:flex-row gap-4">
            ...
            <Select value={filterCategory} onValueChange={setFilterCategory} disabled={isFiltering}>
            ...
            <Select value={filterTier} onValueChange={setFilterTier} disabled={isFiltering}>
              <SelectTrigger className="w-full sm:w-[180px]">
            ...
            <Select value={filterStatus} onValueChange={setFilterStatus} disabled={isFiltering}>
              <SelectTrigger className="w-full sm:w-[180px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                ...
                {filteredBadges.length === 0 ? (
                  ...
                  filteredBadges.map((badge) => (
                    <TableRow key={badge.id}>
                      ...
                      <TableCell>
                        <span className="font-medium">{badge.points_reward}</span>
                      </TableCell>
                      <TableCell>
                        <div className="text-sm">
                          <div className="font-medium">{badge.earned_count} veces</div>
          ...
          {filteredBadges.length > 0 && (
            <div className="mt-4 text-sm text-muted-foreground">
              Mostrando {filteredBadges.length} {filteredBadges.length === 1 ? 'badge' : 'badges'}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Estás seguro de eliminar este badge?</AlertDialogTitle>
            <AlertDialogDescription className="space-y-3">
              <p>
                Estás a punto de eliminar el badge <strong>"{badgeToDelete?.name}"</strong>.
              </p>
              <div className="rounded-lg bg-muted p-3 space-y-1">
                ...
                <ul className="text-sm space-y-1">
                  ...
                  <li>• Estado: {badgeToDelete?.is_active ? 'Activo' : 'Inactivo'}</li>
                </ul>
              </div>
              ...
              <p className="text-sm">
                Los usuarios que ya ganaron este badge lo conservarán en sus perfiles,
                pero no se podrá ganar más en el futuro.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
export function CourseManagementPanel({ academy, loading = false }: CourseManagementPanelProps) {
  ...
  const { 
    ...
    ...academyAdminQueries.courses(academy.id, filters),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['academy-admin', 'courses', academy.id] })
    },
  })
  ...
  const handleDeleteCourse = async (courseId: number) => {
    if (window.confirm('Are you sure you want to delete this course? This action cannot be undone.')) {
      try {
        await deleteMutation.mutateAsync({ academyId: academy.id, courseId })
      } catch (error) {

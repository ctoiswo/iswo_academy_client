...
import { MoreHorizontal, Search, ShieldCheck, GraduationCap, UserCheck, Trash2 } from 'lucide-react'
import academyAdminService, { type AcademyUserRole, type AcademyUser } from '@/services/academy-admin-service'
...
const roleVariant: Record<AcademyUserRole, 'default' | 'secondary' | 'outline'> = {
  admin: 'default',
  teacher: 'secondary',
  student: 'outline',
}
...
export function AcademyUsersPage({ defaultRole = 'all' }: AcademyUsersPageProps) {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: ({ userId, newRole }: { userId: number; newRole: AcademyUserRole }) =>
  ...
  const removeUserMutation = useMutation({
    mutationFn: (userId: number) => academyAdminService.removeUser(slug, userId),
    onSuccess: () => {
  ...
  return (
    ...
    >
      <div className='space-y-4'>
        {/* Role tabs */}
        <Tabs value={role} onValueChange={(v) => { setRole(v as FilterRole); setPage(1) }}>
          <TabsList>
            <TabsTrigger value='all'>
              {t('academyUsers.tabs.all')}
            </TabsTrigger>
            <TabsTrigger value='admin'>
              {t('academyUsers.tabs.admins')}
              {roleCounts && (
                <span className='ml-1.5 text-xs opacity-70'>{roleCounts.admin}</span>
              )}
            </TabsTrigger>
            <TabsTrigger value='teacher'>
              {t('academyUsers.tabs.teachers')}
              {roleCounts && (
                <span className='ml-1.5 text-xs opacity-70'>{roleCounts.teacher}</span>
              )}
            </TabsTrigger>
            <TabsTrigger value='student'>
              {t('academyUsers.tabs.students')}
              {roleCounts && (
                <span className='ml-1.5 text-xs opacity-70'>{roleCounts.student}</span>
              )}
            </TabsTrigger>
          </TabsList>
        </Tabs>
              ...
              {isLoading
                ...
                : users.map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell className='text-muted-foreground text-sm'>
                        {u.enrollments != null && `${u.enrollments} ${t('academyUsers.enrollments')}`}
                        {u.courses_teaching != null && `${u.courses_teaching} ${t('academyUsers.courses')}`}
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align='end'>
                            ...
                            {u.role !== 'teacher' && (
                              <DropdownMenuItem
                                onClick={() =>
                                  updateRoleMutation.mutate({ userId: u.id, newRole: 'teacher' })
                                }
                              >
                            ...
                            {u.role !== 'student' && (
                              <DropdownMenuItem
                                onClick={() =>
                                  updateRoleMutation.mutate({ userId: u.id, newRole: 'student' })
                                }
                              >
      ...
      <AlertDialog open={!!userToRemove} onOpenChange={(open) => !open && setUserToRemove(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('academyUsers.removeDialog.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('academyUsers.removeDialog.description', { name: userToRemove?.name ?? '' })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

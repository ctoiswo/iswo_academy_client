...
const CATEGORIES = ['milestone', 'social', 'mastery', 'streak', 'special', 'collection', 'creator']
...
function BadgeFormDialog({ open, onClose, badge, academySlug }: BadgeFormDialogProps) {
  ...
  const { register, handleSubmit, control, watch, formState: { errors } } = useForm<BadgeSchemaValues>({
    resolver: zodResolver(badgeSchema) as Resolver<BadgeSchemaValues>,
    defaultValues,
  })

  const createMutation = useMutation({
    mutationFn: (data: BadgeFormData) => academyBadgesService.createBadge(academySlug, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['academy-badges', academySlug] })
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEdit ? t('academyBadges.editBadge') : t('academyBadges.newBadge')}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-2">
          ...
          {/* Description */}
          <div className="space-y-1">
            <Label htmlFor="description">{t('academyBadges.form.description')}</Label>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange}>
                    ...
                    <SelectContent>
                      {CATEGORIES.map((c) => (
                        <SelectItem key={c} value={c}>{t(`academyBadges.categories.${c}`)}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                )}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange}>
                    ...
                    <SelectContent>
                      {TIERS.map((t_) => (
                        <SelectItem key={t_} value={t_}>{t(`academyBadges.tiers.${t_}`)}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                )}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange}>
                    ...
                    <SelectContent>
                      {RARITIES.map((r) => (
                        <SelectItem key={r} value={r}>{t(`academyBadges.rarities.${r}`)}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label htmlFor="points_reward">{t('academyBadges.form.pointsReward')}</Label>
              <Input id="points_reward" type="number" min="0" {...register('points_reward', { valueAsNumber: true })} />
            </div>
            <div className="space-y-1">
              <Label htmlFor="display_order">{t('academyBadges.form.displayOrder')}</Label>
          ...
          <div className="flex gap-8">
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="is_active">{t('academyBadges.form.isActive')}</Label>
            </div>
            <div className="flex items-center gap-2">
              <Controller
                name="is_secret"
              ...
              />
              <Label htmlFor="is_secret">{t('academyBadges.form.isSecret')}</Label>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <p className="text-sm font-medium">{t('academyBadges.form.trigger')}</p>
            <div className="space-y-1">
              <Label htmlFor="trigger_type">{t('academyBadges.form.triggerType')}</Label>
              <Input
                id="trigger_type"
            ...
            {watch('trigger_type') && (
              <div className="space-y-1">
                <Label htmlFor="trigger_conditions_raw">{t('academyBadges.form.triggerConditions')}</Label>
                <Textarea
                ...
                />
                <p className="text-xs text-muted-foreground">{t('academyBadges.form.triggerConditionsHint')}</p>
              </div>
            )}
          </div>
    ...
    <Card className="group relative overflow-hidden transition-shadow hover:shadow-md">
      ...
      <CardContent className="px-4 pb-4 space-y-2">
        ...
        <div className="flex flex-wrap gap-1">
          <Badge variant={tierVariant[badge.tier] ?? 'outline'} className="text-xs gap-1">
            {tierIcon[badge.tier]}
            {badge.tier}
          </Badge>
          <Badge variant="outline" className="text-xs capitalize">{badge.rarity}</Badge>
          <Badge variant="secondary" className="text-xs capitalize">{badge.category}</Badge>
          {badge.is_secret && (
            <Badge variant="destructive" className="text-xs">{t('academyBadges.secret')}</Badge>
          )}
        </div>
...
export function AcademyBadgesPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => academyBadgesService.deleteBadge(academySlug!, id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['academy-badges', academySlug] })
  ...
  return (
    <DashboardLayout user={user} variant="full">
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">{t('academyBadges.subtitle')}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Select value={status} onValueChange={setStatus}>
            ...
            <SelectContent>
              <SelectItem value="all">{t('academyBadges.all')}</SelectItem>
              <SelectItem value="active">{t('academyBadges.active')}</SelectItem>
              <SelectItem value="inactive">{t('academyBadges.inactive')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(v) => !v && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('academyBadges.deleteDialog.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('academyBadges.deleteDialog.description', { name: deleteTarget?.name ?? '' })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('academyBadges.deleteDialog.cancel')}</AlertDialogCancel>
            <AlertDialogAction

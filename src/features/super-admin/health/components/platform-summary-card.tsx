...
export function PlatformSummaryCard({ health }: PlatformSummaryCardProps) {
  const stats = [
    ...
    {
      label: 'Academias',
      value: health.total_academies,
      icon: Building2,
      color: 'text-purple-500',
    },
    {
      label: 'Cursos',
      value: health.total_courses,
      icon: BookOpen,
      color: 'text-emerald-500',
    },
    {
      label: 'Pagos totales',
      value: health.total_payments,
      icon: CreditCard,
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className='text-muted-foreground mt-4 flex items-center justify-between border-t pt-4 text-sm'>
          <span>
            Pagos hoy:{' '}
            <strong className='text-foreground'>{health.payments_today}</strong>
          </span>
          <span>
            Ingresos hoy:{' '}
            <strong className='text-foreground'>
              {formatCurrency(health.revenue_today)}
            </strong>
          </span>
        </div>
      </CardContent>

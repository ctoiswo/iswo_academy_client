 ...
 */
function getNavigationItems(dashboardType: DashboardType, academyId?: number): NavigationItem[] {
...
export function RoleBreadcrumb({ academy, currentPath, user, className }: BreadcrumbProps) {
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    pathSegments.forEach((segment, index) => {
      ...
      items.push({
        label,
        path: index === pathSegments.length - 1 ? undefined : currentSegmentPath
      })
    })
        ...
        {breadcrumbItems.map((item, index) => (
          <li key={index} className="flex items-center space-x-2">
            ...
            {item.path ? (
              ...
              >
                {item.icon && <item.icon className="h-3 w-3" aria-hidden="true" />}
                {item.label}
              </Link>
            ) : (

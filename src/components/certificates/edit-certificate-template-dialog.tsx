...
export function EditCertificateTemplateDialog({
  open,
  onOpenChange,
  academySlug,
  templateId,
}: Props) {
  const { data: template, isLoading } = useCertificateTemplate(
    academySlug,
    templateId
  )
  const updateTemplate = useUpdateCertificateTemplate(academySlug, templateId)
  ...
  useEffect(() => {
    ...
    form.reset({
      ...
      layout: (d.layout as 'portrait' | 'landscape') || 'landscape',
      background_color: d.background_color || '#ffffff',
      border_style: d.border_style || 'classic',
      font_family: d.font_family || 'Georgia, serif',
      logo_position: d.logo_position || 'top-center',
      signature_count: d.signature_count || 1,
      title: c.title || 'Certificado de Finalización',
      subtitle: c.subtitle || '',
      body: c.body || '',
      footer: c.footer || '',
      lessons_completion: r.lessons_completion ?? 100,
      minimum_score: r.minimum_score ?? 70,
    })

    if (Array.isArray(c.signatures) && c.signatures.length > 0) {
      setSignatures(c.signatures)
    }
  }, [template, form])
  ...
  const addSignature = () => {
    if (signatures.length < 4) {
      const next = [...signatures, { title: '', name_placeholder: '' }]
      setSignatures(next)
      form.setValue('signature_count', next.length)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-h-[90vh] max-w-3xl overflow-y-auto'>
        <DialogHeader>
          ...
          <DialogDescription>
            Modifica la configuración de esta plantilla
          </DialogDescription>
        </DialogHeader>
            ...
            >
              <Tabs defaultValue='basic' className='w-full'>
                ...
                <TabsContent value='design' className='space-y-4'>
                  ...
                  <FormDescription>
                    El logo del certificado se toma automaticamente del logo de
                    la academia.
                  </FormDescription>

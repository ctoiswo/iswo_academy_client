...
import { useAcademyPermissions, useGlobalPermissions } from '../use-academy-permissions'
...
describe('useAcademyPermissions', () => {
  const mockAcademyData = {
    count: 3,
    academies: [
      { id: 1, name: 'Tech Academy', user_role: 'admin', user_role_display: 'Administrator' },
      { id: 2, name: 'Art Academy', user_role: 'teacher', user_role_display: 'Teacher' },
      { id: 3, name: 'Music Academy', user_role: 'student', user_role_display: 'Student' }
    ]
  }
  ...
  describe('user management permissions', () => {
    it('should allow admin to manage other users', () => {
      ...
      expect(result.current.checkAccess.userManagement('student', false)).toBe(true)
      expect(result.current.checkAccess.userManagement('teacher', false)).toBe(true)
      expect(result.current.checkAccess.userManagement('admin', false)).toBe(true)
    })

    it('should allow users to manage themselves', () => {
      const { result } = renderHook(() => useAcademyPermissions(3))
      
      expect(result.current.checkAccess.userManagement('student', true)).toBe(true)
    })

    it('should deny non-admin from managing other users', () => {
      const { result } = renderHook(() => useAcademyPermissions(3))
      
      expect(result.current.checkAccess.userManagement('teacher', false)).toBe(false)
    })
  })
  ...
  describe('helper functions', () => {
    it('should get academies with specific role', () => {
      ...
      const adminAcademies = result.current.helpers.getAcademiesWithRole('admin')
      expect(adminAcademies).toHaveLength(1)
      expect(adminAcademies[0].id).toBe(1)

      const teacherAcademies = result.current.helpers.getAcademiesWithRole('teacher')
